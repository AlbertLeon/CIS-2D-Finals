...
//      Initialize the logging components
    ...
    public void onInit(int status) {
        if (status == TextToSpeech.SUCCESS) {
            int result = tts.setLanguage(Locale.US); //this is adding english language

            if (result == TextToSpeech.LANG_MISSING_DATA ||
                    result == TextToSpeech.LANG_NOT_SUPPORTED) {
                displayMessage("This language is not supported"); //error message
            } else {
                ttsInitialized = true;
            }
        } else {
            displayMessage("TTS initialization failed"); //error message
        }
    }
